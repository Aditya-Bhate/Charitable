...
// Set TCP timeout to 30 seconds
    ...
    if (strcmp($res, "VERIFIED") == 0 || strcasecmp($res, "VERIFIED") == 0) {
        ...
        $donation = Donation::where('campid',$_POST['custom'])
            ->where('donation_number',$_POST['item_number']);
        $data['txn_id'] = $_POST['txn_id'];
        $data['status'] = $_POST['payment_status'];
        $donation->update($data);

        $campaign = Campaign::findOrFail($_POST['custom']);
        $datas['available_fund'] = $campaign->available_fund + $_POST['mc_gross'];
        $campaign->update($datas);
